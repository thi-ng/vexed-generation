...
struct vgPTF {
    ...
    int add_sweep_point(int idx; const vector profile; string scaleRamp; float twistAmp, twistPhase) {
        ...
        vector cp = set(
            p.x * n.x + p.y * bn.x + q.x,
            p.x * n.y + p.y * bn.y + q.y,
            p.x * n.z + p.y * bn.z + q.z
        );
        return addpoint(0, cp);
    }

    void sweep_lines(const vector profile[]; string scaleRamp; float twistAmp, twistPhase) {
        int numProfile = len(profile);
        for(int i = 0; i < numProfile; i++) {
            int prim = addprim(geo, "polyline");
            int first;
            for(int j = 0; j <= maxPoints; j++) {
                int p = this->add_sweep_point(j, profile[i], scaleRamp, twistAmp, twistPhase);
    ...
    void sweep_tube(const vector profile[]; string scaleRamp; float twistAmp, twistPhase; int close) {
        int numProfile = len(profile);
        int prev[], first[];
        for(int j = 0; j <= maxPoints; j++) {
            int curr[];
            ...
            for(int i = 0; i < numProfile; i++) {
                curr[i] = this->add_sweep_point(j, profile[i], scaleRamp, twistAmp, twistPhase);
            }
            if (j > 0) {
    ...
    void sweep_discs(const vector profile[]; string scaleRamp; float twistAmp, twistPhase; int close) {
        int numProfile = len(profile);
        for(int j = 0; j <= maxPoints; j++) {
            int prev, first;
            int c = addpoint(geo, points[j]);
            for(int i = 0; i < numProfile; i++) {
                int curr = this->add_sweep_point(j, profile[i], scaleRamp, twistAmp, twistPhase);
                if (i > 0) {
